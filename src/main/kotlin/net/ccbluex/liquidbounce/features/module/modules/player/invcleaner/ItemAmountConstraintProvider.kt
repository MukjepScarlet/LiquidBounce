...
interface ItemAmountConstraintProvider {
    ...
    fun hasNonDefaultConstraints(item: ItemFacet): Boolean {
        return getConstraints(item).any { !it.default }
    }
     ...
     */
    fun getApplyingConstraints(
        item: ItemFacet,
        useDefaultConstraints: Boolean = true,
    ): ArrayList<ItemConstraintInfo> {
        val constraints = getConstraints(item)

        if (!useDefaultConstraints || constraints.any { !it.default }) {
