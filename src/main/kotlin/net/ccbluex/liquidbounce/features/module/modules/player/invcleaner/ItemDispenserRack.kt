    ...
    private val dispensersForType = buildMap {
        for (facet in itemFacets) {
            ...
            for (id in wishGroupsForFacet) {
                this.getOrPut(id, ::ArrayDeque).add(facet)
            }
        }

        this.values.forEach { facetList -> facetList.sortDescending() }
    }

    fun nextItemForGroup(id: WishOrganizer.WishItemGroupId): ItemFacet? {
        val dispenser = this.dispensersForType[id] ?: return null
        while (!dispenser.isEmpty()) {
            val currentItem = dispenser.removeFirst()
